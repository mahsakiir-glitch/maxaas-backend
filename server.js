...
const app = express();
app.use(helmet());
app.use(cors({ origin: '*' })); // Production-ka replace garee domain-kaaga
...
// 1. FIXED JWT SECRET (No more random changes!)
...
// --- DATA ROUTES (Based exactly on your schema) ---
...
app.get('/api/v1/quotes', async (req, res) => { const { data, error } = await supabase.from('quotes').select('*'); if (error) return res.status(500).json({ error }); res.json(data); });
app.post('/api/v1/quotes', authMiddleware, async (req, res) => { const { data, error } = await supabase.from('quotes').insert([req.body]).select(); if (error) return res.status(500).json({ error }); res.status(201).json(data[0]); });
app.delete('/api/v1/quotes/:id', authMiddleware, async (req, res) => { const { error } = await supabase.from('quotes').delete().eq('id', req.params.id); if (error) return res.status(500).json({ error }); res.status(204).send(); });
