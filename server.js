...
// Muhiim u ah Render si looga fogaato ValidationError (Rate Limit)
...
// 1. FIXED JWT SECRET (Si uusan qofka uga bixin nidaamka mar kasta)
...
// Ka soo rari aqoonsiga Database-ka marka server-ku kaco
async function loadCredentials() {
    try {
        ...
        console.log("Error loading credentials from DB");
    }
}
loadCredentials();

// Middleware-ka Amniga
...
app.post('/api/v1/auth/login', loginLimiter, async (req, res) => {
    ...
    if (email === currentCredentials.email && 
        bcrypt.compareSync(password, currentCredentials.passwordHash) && 
...
app.post('/api/v1/videos', authMiddleware, async (req, res) => { 
    const { data, error } = await supabase.from('videos').insert([req.body]).select(); 
    if (error) return res.status(500).json({ error }); 
...
app.put('/api/v1/settings', authMiddleware, async (req, res) => { 
    try { 
        for (const [k, v] of Object.entries(req.body)) { 
            await supabase.from('settings').upsert({ key: k, value: v }, { onConflict: 'key' }); 
        } 
...
app.get('/api/v1/stream/:id', optionalAuth, async (req, res) => {
    const { id } = req.params;
    const { data, error } = await supabase.from('videos').select('url').eq('id', id).single();
    if (error || !data || !data.url) return res.status(404).send('Not found');

    let streamUrl = data.url;
    // Archive.org & IPFS support logic...
    if (streamUrl.includes('archive.org/details/')) { 
        const p = streamUrl.split('/details/'); 
        streamUrl = `https://archive.org/download/${p[1]}/${p[1]}.mp4`; 
    }
